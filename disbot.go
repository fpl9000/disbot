...
func getSystemPrompt() string {
    ...
    return fmt.Sprintf("Today's date is %s. You are a helpful assistant that provides concise and " +
                       ...
                       "sentences. Your user is one of a set of people connected to a Discord server " +
                       "(as are you), but you cannot distinguish one user from another. Your output " +
                       "must use Discord markdown so that it renders correctly.", todaysDate)
}
